...
void main()
{
    ...
    ***/
    ...
    vec3 v3f_l2v = v2f_ec_vertex - light_position;
    vec3 v3f_v2l = light_position - v2f_ec_vertex;

    if (length(v3f_l2v) < 1.01f + texture(shadow_map, v3f_l2v).r)
    {
        float n_dot_l = dot(N, normalize(v3f_v2l));
        if (n_dot_l > 0)
        {
            color += light_color * diffuse_color * n_dot_l;
        }

        vec3 v2f_view = normalize((v2f_ec_vertex) - normalize(v3f_v2l));
        float r_dot_v = dot(reflect(normalize(v3f_v2l), N), v2f_view);
