...
keyboard(int key, int scancode, int action, int mods)
{
    if (action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        ...
        switch (key)
        {
            ...
            case GLFW_KEY_9:
            {
                dist_factor_ = fmin(dist_factor_ + 0.5f, 20.0f);
                break;
            }

            case GLFW_KEY_8:
            {
                dist_factor_ = fmax(dist_factor_ - 0.5f, 2.5f);
                break;
            }
...
void Solar_viewer::update_body_positions() {
    ...
    Planet* planets_[]{ &mars_, &venus_, &earth_, &mercury_ };

    for (size_t i = 0; i < 4; i++)
    {
        planets_[i]->pos_ = mat4::rotate_y(planets_[i]->angle_orbit_) * vec4(planets_[i]->distance_, 0, 0, 1);
    }
...
void Solar_viewer::paint()
{
    ...
    if(in_ship_)
    {
        ...
        radius = 8.0f * ship_.radius_;
        mat4 rot = mat4::rotate_y(y_angle_ + ship_.angle_) * mat4::rotate_x(8.0f);
    ...
    else
    {
        ...
        mat4 rot = mat4::rotate_x(y_angle_) * mat4::rotate_y(x_angle_);
        eye = center + rot * vec4(0, 0, (dist_factor_ * radius), 0);
        up = rot * vec4(0, 1, 0, 0);
    }
...
void Solar_viewer::draw_scene(mat4& _projection, mat4& _view)
{
    ...
    Planet* planets_[] = {&stars_, &moon_, &mars_, &earth_, &venus_, &mercury_};

    for (size_t i = 0; i < 6; i++)
    {
        m_matrix = mat4::translate(planets_[i]->pos_) * mat4::rotate_y(planets_[i]->angle_self_) * mat4::scale(planets_[i]->radius_);
        mv_matrix = _view * m_matrix;
        mvp_matrix = _projection * mv_matrix;
        color_shader_.use();
